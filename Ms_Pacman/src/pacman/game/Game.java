...
public final class Game {
	 ...
	 * been played. The variable is set by the methods _feast() and
	 * _checkLevelState().
	 *
	...
	public MOVE directionToClosesPill() {
		...
		return getNextMoveTowardsTarget(current, getClosestNodeIndexFromNodeIndex(current, targetsArray, DM.PATH),
				DM.PATH);
		// return 0;
	...
	public boolean isGhostClose(int distTolerance) {
		...
		if(distanceToClosestGhost()<distTolerance) return true;
//		for (GHOST ghost : GHOST.values())
...
//				}
	...
	public GHOST closestGhost() {
		...
		GHOST closestGhost = null; // Default
		for (GHOST ghost : GHOST.values())
			if (getGhostLairTime(ghost) == 0) {
	...
	public GHOST furthestGhost() {
		int current = getPacmanCurrentNodeIndex();
		int currentFurthest = Integer.MIN_VALUE;
		GHOST FurthestGhost = null; // Default
		for (GHOST ghost : GHOST.values())
			if (getGhostLairTime(ghost) == 0) {
				int tempDistance = getShortestPathDistance(current, getGhostCurrentNodeIndex(ghost));
				if (tempDistance > currentFurthest) {
					FurthestGhost = ghost;
					currentFurthest = tempDistance;
				}
			}
	...
	public MOVE directionToClosestPP() {
		...
		if (targetsArray.length == 0)
			return this.getPacmanLastMoveMade();
		// return the next direction once the closest target has been identified
		return getNextMoveTowardsTarget(current, getClosestNodeIndexFromNodeIndex(current, targetsArray, DM.PATH),
				DM.PATH);
	}
	...
	public boolean isClosestGhostEdible() {
		GHOST closestGhost = closestGhost();
		if(closestGhost==null) return false;
		
		if (this.isGhostEdible(closestGhost))
			return true;
	...
	public STRATEGY getStrategy(int distTolerance) {
		...
		if(ghostClose) {
			...
			if (isPowerPillClose(distClosestGhost))
				return STRATEGY.EATPOWERPILL;
